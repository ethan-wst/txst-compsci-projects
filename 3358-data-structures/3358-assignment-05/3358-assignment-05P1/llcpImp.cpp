...
// definition of MakeOddsAndEvensMeshedAtFront of Assignment 5 Part 1
// ( ***** put at near top to facilitate printing and grading ***** )

void MakeOddsAndEvensMeshedAtFront(Node* headPtr) {
   if (headPtr == 0 || headPtr->link == 0) return;

   Node *cursor, *precursor;
   while (headPtr->link != 0) {
         if (headPtr->data % 2 == 0 && headPtr->link->data % 2 == 0) {

            precursor = headPtr ->link;
            if (precursor->link != 0) cursor = precursor -> link; else return;
            
            while (cursor->data % 2 == 0) {
               if (cursor->link == 0) return;
               precursor = cursor;
               cursor = cursor ->link;
            } 
         ...
         } else if (headPtr->data % 2 == 1 && headPtr->link->data % 2 == 1) {

            precursor = headPtr ->link;
            if (precursor->link != 0) cursor = precursor -> link; else return;

            while (cursor->data % 2 == 1) {
               if (cursor->link == 0) return;
               precursor = cursor;
               cursor = cursor ->link;
            } 
