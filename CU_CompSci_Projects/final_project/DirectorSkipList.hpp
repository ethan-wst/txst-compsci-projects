...
struct DirectorSLNode {
  ...
  DirectorSLNode(string _director, int levels) {
    director = _director;
    //creates a vector of size levels with all values of nullptr, need to set these to point to the next Node
...
class DirectorSkipList {
    ...
    public:
        ...
        // Calculates the random number of levels a new node should have
        int CalculateLevels();
};
